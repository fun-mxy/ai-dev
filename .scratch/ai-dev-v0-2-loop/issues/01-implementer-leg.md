...
- [x] 从已冻结 tasks + lane-graph 的 feature run，为 `LANE-001` 的一个 task 构建 implementer input package（task 文本来自 `03-tasks.md`、allowed-files 来自 lane-graph）
- [x] 复用 v0.1 `prepare-run` / `run-headless` / `validate-run` 跑通一次 implement run（不重建 run 机制）
- [x] `result.json` 的 task 状态写回 canonical `task-status.yml` = `proposed_done`；由 deterministic runtime 写、不经模型（§4.3）
- [x] 产出 lane 级 `implement-result.{md,json}`，字段齐全且与 run 的 result+metadata 一致（md+json 双产物 §4.4）
- [x] §9.2 限制守住：不宣布 final done、不动 frozen、不越 allowed-files boundary（复用 `validate-run`）
- [x] 单测：input-package 从冻结 artifact 组装、`proposed_done` 写回、`implement-result` rollup
