...
- [x] 解析指定 profile，含 §10.1 全部字段
- [x] token 仅按名解析（auth_env -> fallback），配置无 token 值（不变量 #11）
- [x] `show-profile` 输出 token 值脱敏；任何输出路径不泄露 token
- [x] 缺 profile / 缺 token 来源 -> fail loud（§24.2），退出码非 0
- [x] 单测：合法解析、fallback 触发、token 不泄露
