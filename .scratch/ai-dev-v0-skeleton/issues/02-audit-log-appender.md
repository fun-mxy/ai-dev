...
- [ ] 提供 append 语义：给定事件类型与结构化负载，追加一条带时间戳与事件类型的事件
- [ ] 每次追加同时写入 Markdown（人读）与 JSON（机读）两个产物（§4.4）
- [ ] append-only：既有条目不可被篡改或删除
- [ ] 01 中 inline 的审计写入改由本组件承担
- [ ] 连续追加多条不同类型事件，md 与 json 内容一致且完整
