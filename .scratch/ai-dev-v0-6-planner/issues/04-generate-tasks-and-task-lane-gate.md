...
- [ ] `generate-tasks` command: Planner role, input package = intent + frozen requirements + frozen design
- [ ] run emits id-free tasks proposal (TASK slots, local REQ+DES refs, lane assignment, expected/exclusive files, purpose)
- [ ] `promote` allocates TASK ids + stitches against frozen requirements **and** design, writing all four: `03-tasks.json` (**new**), rendered `03-tasks.md`, seeded `task-status.yml` (all `pending`), populated `04-lane-graph.yml`
- [ ] task/lane gate coverage precheck: every REQ+DES referenced by ≥1 task, else refuse to freeze
- [ ] freeze freezes `03-tasks(.json+.md)` + `04-lane-graph.yml` together; advances to `lane_gate`
- [ ] refinement loop (`--feedback`); real cc-glm52/Ark evidence; `uv run mypy` + `uv run pytest` green
