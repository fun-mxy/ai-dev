...
- [ ] `fix_loop_budget: {used, max: 1}` 写入 feature-status，初 used=0
- [ ] apply_triage `request_fix` 在 used>=max 时 fail-loud 拒绝
- [ ] `ai-dev fix-run` 驱动一次 fix implement run
- [ ] budget 仅在 §14-validated implement-result 自增（boundary/frozen/crash 不消耗）
- [ ] §24.3 schema auto-retry 不消耗 budget（run 内部）
- [ ] request_fix reappear-after-own-fix -> status=reappeared -> triage 失效 -> 强制 re-triage
- [ ] §19 flow amend：re-triage 步骤显式画出
- [ ] 脏 worktree（boundary/frozen）提示人工 revert（§24.4），不自动重 launch
- [ ] 单测：budget 0->1、超限拒绝、crash 不消耗、reappear 触发 re-triage
