...
The `origin=dry-run` emission is **deferred**. Ticket 02's `origin` parameter on
`append_audit_event` has now landed (the `origin` glossary row enumerates
`dry-run` as a valid value, and the gate/report writers thread `origin=` through);
dry-run still does not emit, because "feature 树零改动" rules out an audit append
even for traceability. A follow-up can flip dry-run to emit one
`origin=dry-run` audit event per plan without revisiting this ADR's skip points.
